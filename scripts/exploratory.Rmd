...
Before running the following you'll need to set up the materialize views

General materialize views from the mimic github repo

```bash
MIMICREPO=/path/to/your/mimic/repo
cd $MIMICREPO/concepts
psql --username <urusername --dbname mimic 
```
...
Private scripts from Alistaire

```bash
TIDAL=/path/to/tidal/volume/mimic/repo
cd $TIDAL/scripts/sql
psql --username <urusername --dbname mimic
```
